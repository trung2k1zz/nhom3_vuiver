...
$res=mysql_query("select * from gkck gc join totruongchuyenmon tt on tt.MaTTCM=gc.MaTTCM
join giaovien as gv on gv.MaGV=tt.MaGV join truong as tr on tr.MaTruong=gv.MaTruong 
where gc.TieuDe='$tieude' and gc.MaMonHoc='$mamon' and tr.MaTruong='$matruong' Order By Rand()");?>
<?php
...
$malop=$_GET['MaLop'];
$tieude=$_GET['TieuDe'];
$response=mysql_query("select * from gkck gc join totruongchuyenmon tt on tt.MaTTCM=gc.MaTTCM
join giaovien as gv on gv.MaGV=tt.MaGV join truong as tr on tr.MaTruong=gv.MaTruong 
where gc.TieuDe='$tieude' and gc.MaMonHoc='$mamon' and tr.MaTruong='$matruong' Order By Rand()");?>

<form method='post' id='quiz_form' action='KetQuaLamBaiGK.php' name='form'>

<?php
$a=1;
while($result=mysql_fetch_assoc($response)){ ?>
<div id="question_<?php echo $result['MaGKCK'];?>" class='questions'>
<h4><div id="question_<?php echo $result['MaGKCK'];?>"><?php $result['MaGKCK']?>&nbsp;<?php echo $a++;?>.<?php
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="A" id='radio1_<?php echo $result['MaGKCK'];?>' name='<?php echo $result['MaGKCK'];?>'
<label id='ans1_<?php echo $result['MaGKCK'];?>' for='1'><?php
							$anh=$result['QA'];
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="B" id='radio2_<?php echo $result['MaGKCK'];?>' name='<?php echo $result['MaGKCK'];?>'
<label id='ans2_<?php echo $result['MaGKCK'];?>' for='1'><?php
							$anh=$result['QB'];
							if(!preg_match("/^[A-Za-z]{1,100}[.(jpg|png)]{3}/",$anh)){
									echo "<k>"."&nbsp;B.".$result['QB']."<br/>"."</k>";
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="C" id='radio3_<?php echo $result['MaGKCK'];?>' name='<?php echo $result['MaGKCK'];?>'
<label id='ans3_<?php echo $result['MaGKCK'];?>' for='1'><?php
							$anh=$result['QC'];
							if(!preg_match("/^[A-Za-z]{1,100}[.(jpg|png)]{3}/",$anh)){
								...
								?></label>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="D" id='radio4_<?php echo $result['MaGKCK'];?>' name='<?php echo $result['MaGKCK'];?>'
<label id='ans4_<?php echo $result['MaGKCK'];?>' for='1'><?php
							$anh=$result['QD'];
							if(!preg_match("/^[A-Za-z]{1,100}[.(jpg|png)]{3}/",$anh)){
...
<input type="radio" checked='checked' value="5" style='display:none' id='radio4_<?php echo $result['MaGKCK'];?>' name='<?php echo $result['MaGKCK'];?>'>
<br/>
</div>
<?php }
$a++;
